...
class PairTest extends GroovyTestCase {
   ...
   def quote(time,ticker, price) {
      pair.quotes["$today $time $ticker"] = ["bid":price]
   }
   ...
   void test_calculate_difference_between_current_spread_and_moving_average() {
      ...
      assertEquals(-0.0082, pair.spread(), 0.0001)
   }
   ...
   void test_calculate_moving_average_from_market_open() {
      ...
      assertEquals(-0.0195, pair.spread(), 0.0001)
   }
   
   void test_only_use_current_day_for_spread_calculations() {
      pair.quotes["2011-11-14 16:00:00 SPY"] = ["bid":133.97	]
      pair.quotes["2011-11-14 16:00:00 IVV"] = ["bid":134.4036	]
      pair.quotes["2011-11-15 09:30:00 SPY"] = ["bid":134.01	]
      pair.quotes["2011-11-15 09:30:00 IVV"] = ["bid":134.46	]
      
      assertEquals(0.0, pair.spread())
   }
   ...
   void test_detect_a_high_low_spread() {
      new File("./src/test/resources/fixtures.csv").splitEachLine(",") {
         ...
         if(time_stamp =~ ~"10:59:00") { assertEquals( -0.0319, pair.spread(), 0.0001) }
         if(time_stamp =~ ~"11:09:00") { assertEquals( -0.0319, pair.spread(), 0.0001) }
         if(time_stamp =~ ~"11:40:00") { assertEquals( 0.0359, pair.spread(), 0.0001) }
      }
   }
}